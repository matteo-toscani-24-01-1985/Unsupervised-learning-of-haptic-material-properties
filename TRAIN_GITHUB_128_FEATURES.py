...
ks=5
x = Conv1D(128, kernel_size=ks, strides=(1), padding="same", activation="relu", data_format="channels_last")(input_img) #nb_filter, nb_row, nb_col
x = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)
x = Conv1D(128, kernel_size=ks, strides=(1), padding='same', activation='relu', data_format="channels_last")(x)
x = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)
x = Conv1D(128, kernel_size=ks, strides=(1), padding="same", activation="relu", data_format="channels_last")(x)
x = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)
x = Conv1D(128, kernel_size=ks, strides=(1), padding="same", activation="relu", data_format="channels_last")(x)
...
x = Conv1D(128, kernel_size=ks, strides=( 1), padding='same', activation='relu', data_format="channels_last")(encoded)
...
weights_file = os.path.join( 'autoencoder_texture_weights_LMT_128_compression.hdf5')  # where weights will be saved
json_file = os.path.join( 'autoencoder_texture_model_LMT_128_compression.json')

csv_logger = CSVLogger(os.path.join('training_log_LMT_128_compression.csv'), append=True, separator=';') # added by KS
...
#### save model keras
autoencoder.save('autoencoder_model_128_compression.h5')
encoder.save('encoder_model_128_compression.h5')
...
if not os.path.exists('./recontructed_128_compression/'): os.mkdir('./recontructed_128_compression/')
if not os.path.exists('./Code_128_compression/'): os.mkdir('./Code_128_compression/')
...
with open('imageListLMT_128_compression.txt', 'w') as f:
...
for i in range(X.shape[0]):
    ...
    scipy.io.savemat("{}{}{}".format('./Code_128_compression/', NAMES[i], '.mat'), mdict={'arr': code})

for i in range(X.shape[0]):
    tmp=np.expand_dims(X[i,],0)
    tmp = autoencoder.predict(tmp)
    scipy.io.savemat("{}{}{}".format('./recontructed_128_compression/', NAMES[i], '.mat'), mdict={'arr': tmp})
