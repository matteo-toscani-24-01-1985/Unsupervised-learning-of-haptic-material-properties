...
x = Conv1D(32, kernel_size=ks, strides=(1), padding='same', activation='relu', data_format="channels_last")(x)
x = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)
x = Conv1D(16, kernel_size=ks, strides=(1), padding="same", activation="relu", data_format="channels_last")(x)
x = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)
x = Conv1D(16, kernel_size=ks, strides=(1), padding="same", activation="relu", data_format="channels_last")(x)
encoded = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)

x = Conv1D(16, kernel_size=ks, strides=( 1), padding='same', activation='relu', data_format="channels_last")(encoded)
...
weights_file = os.path.join( 'autoencoder_texture_weights_LMT.hdf5')  # where weights will be saved
json_file = os.path.join( 'autoencoder_texture_model_LMT.json')

csv_logger = CSVLogger(os.path.join('training_log_LMT.csv'), append=True, separator=';') # added by KS
...
if not os.path.exists('./recontructed/'): os.mkdir('./recontructed/')
if not os.path.exists('./Code/'): os.mkdir('./Code/')
...
NAMES=list()  
X = np.zeros(( TotalSamples/2,) + desired_im_sz); # allocate space
counter=0;
for s in range(np.shape(all_frames)[0]):
    filename=all_frames[s]
    im=np.loadtxt(os.path.join(img_dir, filename), dtype='float', comments='#', delimiter=',')
...
for i in range(X.shape[0]):
    ...
    scipy.io.savemat("{}{}{}".format('./Code/', NAMES[i], '.mat'), mdict={'arr': code})

for i in range(X.shape[0]):
    tmp=np.expand_dims(X[i,],0)
    tmp = autoencoder.predict(tmp)
    scipy.io.savemat("{}{}{}".format('./recontructed/', NAMES[i], '.mat'), mdict={'arr': tmp})
