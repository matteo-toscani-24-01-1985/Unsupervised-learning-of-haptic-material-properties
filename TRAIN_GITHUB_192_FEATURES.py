...
ks=5
x = Conv1D(192, kernel_size=ks, strides=(1), padding="same", activation="relu", data_format="channels_last")(input_img) #nb_filter, nb_row, nb_col
x = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)
x = Conv1D(192, kernel_size=ks, strides=(1), padding='same', activation='relu', data_format="channels_last")(x)
x = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)
x = Conv1D(192, kernel_size=ks, strides=(1), padding="same", activation="relu", data_format="channels_last")(x)
x = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)
x = Conv1D(192, kernel_size=ks, strides=(1), padding="same", activation="relu", data_format="channels_last")(x)
...
x = Conv1D(192, kernel_size=ks, strides=( 1), padding='same', activation='relu', data_format="channels_last")(encoded)
...
weights_file = os.path.join( 'autoencoder_texture_weights_LMT_192_compression.hdf5')  # where weights will be saved
json_file = os.path.join( 'autoencoder_texture_model_LMT_192_compression.json')

csv_logger = CSVLogger(os.path.join('training_log_LMT_192_compression.csv'), append=True, separator=';') # added by KS
...
#### save model keras
autoencoder.save('autoencoder_model_192_compression.h5')
encoder.save('encoder_model_192_compression.h5')
...
if not os.path.exists('./recontructed_192_compression/'): os.mkdir('./recontructed_192_compression/')
if not os.path.exists('./Code_192_compression/'): os.mkdir('./Code_192_compression/')
...
for i in range(X.shape[0]):
    ...
    scipy.io.savemat("{}{}{}".format('./Code_192_compression/', NAMES[i], '.mat'), mdict={'arr': code})

for i in range(X.shape[0]):
    tmp=np.expand_dims(X[i,],0)
    tmp = autoencoder.predict(tmp)
    scipy.io.savemat("{}{}{}".format('./recontructed_192_compression/', NAMES[i], '.mat'), mdict={'arr': tmp})
